...
async function getGalleryImages() {
  ...
  try {
    ...
    const imagesWithDates = await Promise.all(
      filenames.map(async (filename) => {
        ...
        try {
          ...
          console.warn(
            `Could not get date for ${filename}, falling back to file stats.`,
            e,
          );
          const stats = await fs.stat(filePath);
          date = stats.mtime;
    ...
    console.warn(
      "Could not read gallery directory. This is normal if the directory doesn't exist.",
      error,
    );
    return [];
  }
