...
export function Chatbot() {
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
        setMessages([
            { role: 'assistant', content: "Hello! I'm an AI assistant. Feel free to ask me anything about Kingsley's profile." }
        ])
    }
  ...
  return (
    <>
      ...
      <div
        className={cn(
          'fixed bottom-24 right-4 z-50 w-[calc(100vw-32px)] max-w-md h-[60vh] overflow-hidden rounded-lg border bg-card shadow-xl transition-all duration-300 ease-in-out',
          isOpen ? 'translate-x-0 opacity-100' : 'translate-x-[calc(100%+32px)] opacity-0'
        )}
      >
